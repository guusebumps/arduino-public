...
//===========================================================================
// Modified by Boian Mitov to reduce memory usage and improve performance on microcontrollers
//===========================================================================
//
//#include "JPEGDEC.h"
...
const uint8_t PROGMEM cZigZag[64] = 
{
    0,1,5,6,14,15,27,28,
...
// un-zigzag ordering
const uint8_t PROGMEM cZigZag2[64] = 
{
    0,1,8,16,9,2,3,10,
...
// IFAST_SCALE_BITS.
const uint16_t PROGMEM iScaleBits[64] = 
{
...
//
/*
const uint8_t PROGMEM ucRangeTable[] = 
{
...
inline uint8_t Get_ucRange( uint16_t AIndex )
{
	if( AIndex < 128 )
		return 0x80 + AIndex;
...
//
/*
const uint16_t PROGMEM usGrayTo565[] = 
{
...
*/
inline uint16_t GetGrayTo565( uint8_t AIndex )
{
    return ( AIndex / 4 ) * ( 0x20 + 0.5 ) + ( AIndex / 8 ) * 0x0800;
}
...
//
/*
const uint16_t PROGMEM usRangeTableR[] = 
{
...
//
const uint16_t PROGMEM usRangeTableG[] = 
{
    0x0000,0x0000,0x0000,0x0000,0x0020,0x0020,0x0020,0x0020, // 0
...
//
const uint16_t PROGMEM usRangeTableB[] = 
{
    0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000,0x0000, // 0
...
inline uint16_t GetRangeR( uint16_t AIndex )
{
	if( AIndex < 256 )
		return( ( AIndex / 8 ) * 0x0800 );
...
inline uint16_t GetRangeG( uint16_t AIndex )
{
	if( AIndex < 256 )
		return( ( AIndex / 4 ) * 0x0020 );
...
inline uint16_t GetRangeB( uint16_t AIndex )
{
	if( AIndex < 256 )
		return( ( AIndex / 8 ) * 0x0001 );
...
static void JPEGFixQuantD(JPEGIMAGE *pJPEG)
{
    ...
    for (iTable=0; iTable<pJPEG->ucNumComponents; iTable++)
    {
        ...
        for (i=0; i<DCTSIZE; ++ i )
            sTemp[i] = p[ pgm_read_byte( cZigZag + i ) ];
        ...
        p = (uint16_t *)&pJPEG->sQuantTable[iTableOffset];
        for (i=0; i<DCTSIZE; ++ i )
            p[i] = (uint16_t)((p[i] * pgm_read_word( iScaleBits + i )) >> 12);
...
static int JPEGDecodeMCU(JPEGIMAGE *pJPEG, int iMCU, int *iDCPredictor)
{
    ...
    const uint8_t AEnd = 64;
    uint8_t AEnd2;

    signed short *pMCU = (short int*)&pJPEG->sMCUs[iMCU];
...
//    cZigZag2
    ...
    if (pJPEG->iOptions & (JPEG_SCALE_QUARTER | JPEG_SCALE_EIGHTH)) // reduced size DCT
    {
        ...
        AEnd2 = 5; // we only need to store the 4 elements we care about
    }
    else
    {
...
//    pEnd = (unsigned char *)&cZigZag2[64];
    ...
    pFast = (short unsigned int*)&pJPEG->usHuffAC[pJPEG->ucACTable * HUFF11SIZE];
...
//                pFast = &pJPEG->ucAltHuff[0];
        ...
        while( AZig < AEnd )
        {
            ...
            AZig += (usHuff >> 4);  // get the skip amount (RRRR)
            usHuff &= 0xf; // get (SSSS) - extra length
            if( AZig < AEnd && usHuff ) // && piHisto)
            {
                ulCode = ulBits << ulBitOff;
                ...
                uint8_t AZigValue = pgm_read_byte( cZigZag2 + AZig );
                ucMaxACCol |= 1<<( AZigValue & 7); // keep track of occupied columns
                if ( AZigValue >= 0x20) // if more than 4 rows used in a col, mark it
                    ucMaxACRow |= 1<<( AZigValue & 7); // keep track of the max AC term row

                pMCU[ AZigValue ] = (signed short)ulCode; // store AC coefficient (already reordered)
            }
...
static void JPEGIDCT(JPEGIMAGE *pJPEG, int iMCUOffset, int iQuantTable, int iACFlags)
{
    ...
    pQuant = (short int*)&pJPEG->sQuantTable[iQuantTable * DCTSIZE];
    if (pJPEG->iOptions & JPEG_SCALE_QUARTER) // special case
    {
        ...
        pOutput[0] = Get_ucRange(((tmp0 + tmp1)>>5) & 0x3ff);
        pOutput[1] = Get_ucRange(((tmp0 - tmp1)>>5) & 0x3ff);
        /* Row 1 */
        pOutput[2] = Get_ucRange(((tmp2 + tmp3)>>5) & 0x3ff);
        pOutput[3] = Get_ucRange(((tmp2 - tmp3)>>5) & 0x3ff);
        return;
    }
    // do columns first
    ...
    for (iRow=0; iRow<64; iRow+=8) // all rows must be calculated
    {
        ...
        pOutput[0] = Get_ucRange(((tmp0 + tmp7)>>5) & 0x3ff);
        pOutput[1] = Get_ucRange(((tmp1 + tmp6)>>5) & 0x3ff);
        pOutput[2] = Get_ucRange(((tmp2 + tmp5)>>5) & 0x3ff);
        pOutput[3] = Get_ucRange(((tmp3 - tmp4)>>5) & 0x3ff);
        pOutput[4] = Get_ucRange(((tmp3 + tmp4)>>5) & 0x3ff);
        pOutput[5] = Get_ucRange(((tmp2 - tmp5)>>5) & 0x3ff);
        pOutput[6] = Get_ucRange(((tmp1 - tmp6)>>5) & 0x3ff);
        pOutput[7] = Get_ucRange(((tmp0 - tmp7)>>5) & 0x3ff);
        pOutput += 8;
    } // for each row
} /* JPEGIDCT() */
...
static void JPEGPutMCUGray(JPEGIMAGE *pJPEG, int x, int iPitch)
{
    ...
    if (pJPEG->iOptions & JPEG_SCALE_HALF) // special handling of 1/2 size (pixel averaging)
    {
        ...
        for (i=0; i<4; i++)
        {
            ...
            else
            {
                ...
                {
                    pix = (pSrc[0] + pSrc[1] + pSrc[8] + pSrc[9] + 2) >> 2; // average 2x2 block
                    usDest[j] = __builtin_bswap16( GetGrayTo565( pix ));
                    pSrc += 2;
                }
            }
            pSrc += 8; // skip extra line
    ...
    for (i=0; i<ycount; i++) // do up to 8 rows
    {
        ...
        {
            for (j=0; j<xcount; j++)
                *usDest++ = GetGrayTo565( *pSrc++ );
        }
        else
        {
            for (j=0; j<xcount; j++)
                *usDest++ = __builtin_bswap16( GetGrayTo565( *pSrc++ ));
        }
        usDest -= xcount;
...
//
    ...
    iCRG = -2925 * (iCr-0x80);
    iCRR = 5742  * (iCr-0x80);
    usPixel = GetRangeB(((iCBB + iY) >> 12) & 0x3ff ); // blue pixel
    usPixel |= GetRangeG(((iCBG + iCRG + iY) >> 12) & 0x3ff ); // green pixel
    usPixel |= GetRangeR(((iCRR + iY) >> 12) & 0x3ff ); // red pixel
    pDest[0] = usPixel;
#endif
} /* JPEGPixelLE() */
...
//    
    ...
    *(uint32_t *)&pDest[0] = ulPixel1;
    ...
    iCRG = -2925 * (iCr-0x80);
    iCRR = 5742  * (iCr-0x80);
    ulPixel1 = GetRangeB(((iCBB + iY1) >> 12) & 0x3ff ); // blue pixel
    ulPixel1 |= GetRangeG(((iCBG + iCRG + iY1) >> 12) & 0x3ff ); // green pixel
    ulPixel1 |= GetRangeR(((iCRR + iY1) >> 12) & 0x3ff ); // red pixel
    
    ulPixel2 = GetRangeB(((iCBB + iY2) >> 12) & 0x3ff ); // blue pixel
    ulPixel2 |= GetRangeG(((iCBG + iCRG + iY2) >> 12) & 0x3ff ); // green pixel
    ulPixel2 |= GetRangeR(((iCRR + iY2) >> 12) & 0x3ff ); // red pixel
    ...
    for (y = 0; y < cy && bContinue && iErr == 0; y++, jd.y += mcuCY)
    {
        ...
        for (x = 0; x < cx && bContinue && iErr == 0; x++)
        {
            ...
            if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
            {
                ...
                c = Get_ucRange(((iDCPred0 * iQuant1) >> 5) & 0x3ff );
            ...
            if (pJPEG->ucSubSample && pJPEG->ucNumComponents == 3) // if color (not CMYK)
            {
                ...
                if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                {
                    c = Get_ucRange(((iDCPred1 * iQuant2) >> 5) & 0x3ff );
                    l = c | ((uint32_t) c << 8) | ((uint32_t) c << 16) | ((uint32_t) c << 24);
                    // dct stores byte values
                ...
                if (pJPEG->ucMaxACCol == 0 || bThumbnail) // no AC components, save some time
                {
                    c = Get_ucRange(((iDCPred2 * iQuant3) >> 5) & 0x3ff );
                    l = c | ((uint32_t) c << 8) | ((uint32_t) c << 16) | ((uint32_t) c << 24);
                    // dct stores byte values
